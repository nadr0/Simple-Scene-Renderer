...
RGBColor Microfacet::area_light_shade(ShadeRec & sr){
    Vec4 wo = -sr.ray.d;
    ...
    for (int j = 0; j < num_lights; j++) {
        ...
        if (ndotwi > 0.0){
            ...
            if(!in_shadow){
                L += (diffuse_brdf->f(sr, wo, wi) + specular_brdf->sample_f(sr,wo,wi))* sr.w->lights[j]->L(sr) * sr.w->lights[j]->G(sr) * ndotwi / sr.w->lights[j]->pdf(sr);
            }
        }
